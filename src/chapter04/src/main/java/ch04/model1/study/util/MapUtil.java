...
public class MapUtil {
    ...
    /**
     * LRU map 생성
     * @param <K> Map의 key 제네릭
     * @param <V> Map의 value 제네릭
     * @param maxEntry
     * @return
     */
    public static <K, V> Map<K, V> makeLRUMap(int maxEntry) {
        return new LRUMap<K, V>(getEntry(maxEntry));
    }
    ...
    public static <K, V> ConcurrentMap<K, V> makeConcurrentLRUMap(int maxEntry) {
        return new ConcurrentLinkedHashMap.Builder<K, V>()
                                          .maximumWeightedCapacity(getEntry(maxEntry))
                                          .build();
    }
